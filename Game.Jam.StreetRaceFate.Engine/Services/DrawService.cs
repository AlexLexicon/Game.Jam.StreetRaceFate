...
public interface IDrawService
{
    ...
    void Draw(SpriteBatch spriteBatch, SpriteFont spriteFont, string text, Vector2 position, Color color, float scale = 1f, float layerDepth = 1f);
}
public class DrawService : IDrawService
{
    ...
    public void Draw(SpriteBatch spriteBatch, SpriteFont spriteFont, string text, Vector2 position, Color color, float scale = 1f, float layerDepth = 1f)
    {
        Vector2 size = spriteFont.MeasureString(text);

        spriteBatch.DrawString(spriteFont, text, new Vector2(position.X - size.X / 2, position.Y - size.Y / 2), color, 0f, Vector2.Zero, scale, SpriteEffects.None, layerDepth);
    }
}
